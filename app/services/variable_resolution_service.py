...
class VariableResolutionService:
    ...
    def resolve_variable(self, 
        ...
        cache_key = f"{user_variable}:{dataset_fingerprint}:{threshold}:{allow_multiple}"

        if cache_key in self._cache:
            logger.debug(f"Cache hit for variable '{user_variable}'")
            return self._cache[cache_key]

        # Optional multi-variable resolution path (splits on commas/"and")
        if allow_multiple:
            parts = [
                p.strip()
                for p in re.split(r',|;|\band\b', user_variable, flags=re.IGNORECASE)
                if p.strip()
            ]
            if len(parts) > 1:
                ...
                for part in parts:
                    sub_result = self.resolve_variable(
                        part,
                        available_columns,
                        threshold=threshold,
                    ...
                    component_results.append(sub_result)
                    if sub_result.get('matched'):
                        matched_vars.append(sub_result['matched'])
                        confidences.append(sub_result.get('confidence', 0.0))
                    elif return_suggestions and sub_result.get('suggestions'):
                        suggestions_map[part] = sub_result['suggestions']

                aggregated_response = {
                    'matched': matched_vars[0] if matched_vars else None,
                ...
                self._cache[cache_key] = aggregated_response
