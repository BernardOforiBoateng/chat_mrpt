        ...
        try:
            ...
            if health['healthy']:
                ...
                class LLMManagerWrapper:
                    ...
                    def generate_with_image(self, prompt, image_url, session_id=None):
                        """Proxy to adapter's generate_with_image method for vision support."""
                        if hasattr(self.adapter, 'generate_with_image'):
                            return self.adapter.generate_with_image(prompt, image_url, session_id)
                        else:
                            # Fallback if adapter doesn't have vision support
                            return self.adapter.generate(prompt, max_tokens=500, temperature=0.7)
