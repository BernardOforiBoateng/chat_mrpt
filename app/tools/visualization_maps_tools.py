...
class CreateCompositeVulnerabilityMap(BaseTool):
    """
    Create vulnerability map specifically for COMPOSITE method only.

    Uses create_agent_vulnerability_map() with composite method to create
    choropleth maps showing wards colored by their composite risk categories.
    """
    ...
    def get_description(cls) -> str:
        return "Create vulnerability map using composite score method only"

    @classmethod
    def get_examples(cls) -> List[str]:
        return [
            "Create composite vulnerability map",
            "Show me a map of composite risk levels",
            "Generate composite method risk map",
            "Create vulnerability map for composite score method"
        ]

    def execute(self, session_id: str) -> ToolExecutionResult:
        """Create composite vulnerability map using existing agent function"""
        try:
            logger.info("Creating composite vulnerability map...")
            ...
            if not exists:
                error_msg = variable_resolver.create_variable_error_message(
                    'composite_score', list(gdf.columns),
                    context="for composite vulnerability mapping. Please run composite analysis first"
                )
                return self._create_error_result(error_msg)
            ...
            map_result = create_agent_vulnerability_map(
                unified_dataset=gdf,
                session_id=session_id,
                method='composite'  # Explicitly use composite method
            )

            if map_result.get('status') == 'error':
                return self._create_error_result(
                    f"Composite vulnerability map creation failed: {map_result.get('message', 'Unknown error')}"
                )
            ...
            result_data = {
                ...
                'web_path': map_result.get('web_path'),
                'file_path': map_result.get('file_path'),
                'map_type': 'composite_vulnerability_map'
            }

            # Check for composite rank info
            if 'composite_rank' in gdf.columns:
                ranked_wards = gdf[gdf['composite_rank'].notna()]
                message = f"Created composite vulnerability map with {len(ranked_wards)} wards classified using composite_rank"
            else:
                message = f"Created composite vulnerability map with {self.risk_categories} risk categories for {len(gdf)} wards"
            ...
            logger.error(f"Error creating composite vulnerability map: {e}")
            return self._create_error_result(f"Composite vulnerability map creation failed: {str(e)}")
...
class CreateVulnerabilityMapComparison(BaseTool):
    ...
    @classmethod
    def get_description(cls) -> str:
        return "Create RISK ASSESSMENT maps showing vulnerability rankings from completed analysis (composite vs PCA comparison). Uses analysis results like composite_rank and pca_rank columns. NOT for mapping raw data variables - use variable_distribution for that."
    ...
    def get_examples(cls) -> List[str]:
        return [
            ...
            "Show both risk assessment methods",
            "Display vulnerability rankings comparison"
        ]
    ...
    def execute(self, session_id: str) -> ToolExecutionResult:
        ...
        try:
            ...
            # Filter out null/empty geometries before creating maps
            # This prevents blank maps when some wards have null geometries
            valid_geometry_mask = ~gdf.geometry.isnull() & ~gdf.geometry.is_empty
            dropped_count = (~valid_geometry_mask).sum()
            if dropped_count > 0:
                logger.warning(f"Filtering out {dropped_count} wards with null/empty geometries from vulnerability comparison map")

            gdf = gdf[valid_geometry_mask].copy()

            if len(gdf) == 0:
                return self._create_error_result("No valid geometries found after filtering. Please check your shapefile data.")
