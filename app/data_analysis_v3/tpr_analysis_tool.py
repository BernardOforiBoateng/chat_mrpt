...
Try using 'all_ages' with 'both' test methods and 'all' facilities for the broadest analysis."""
            ...
            # Simplified result - just the summary
            # The workflow transition function will add the menu
            result = f"""## TPR Analysis Complete

**{state_name}**: {summary['mean_tpr']}% average across {summary['total_wards']} wards"""
            ...
            # CRITICAL: Enhance result message with files created and next steps
            # Check what was actually created and add to message
            files_created = []
            if debug_stages.get("file_creation", {}).get("raw_data", {}).get("success"):
                files_created.append("• `raw_data.csv` - Ward-level TPR + environmental variables")
            if debug_stages.get("file_creation", {}).get("shapefile", {}).get("success"):
                files_created.append("• `raw_shapefile.zip` - Geographic boundaries with attributes")
            if os.path.exists(os.path.join(session_folder, 'tpr_results.csv')):
                files_created.append("• `tpr_results.csv` - Detailed TPR calculations")

            map_created = debug_stages.get("map_creation", {}).get("success", False)

            if files_created or map_created:
                result += "\n\n**Files Created:**\n"
                if files_created:
                    result += "\n".join(files_created)
                if map_created:
                    result += "\n• `tpr_distribution_map.html` - Interactive TPR visualization"

                # Add information about what was prepared
                if debug_stages.get("env_extraction", {}).get("success"):
                    env_vars = debug_stages["env_extraction"].get("variables", [])
                    # Filter out WardCode
                    env_vars = [v for v in env_vars if v != 'WardCode']
                    if env_vars:
                        result += f"\n\n**Environmental Variables Added:** {', '.join(env_vars[:5])}"
                        if len(env_vars) > 5:
                            result += f" (and {len(env_vars) - 5} more)"

                # Add next steps guidance
                result += "\n\n**What You Can Do Next:**\n\n"
                result += "Ask me questions about the TPR data, or type:\n"
                result += "- **'Show columns'** - View all data columns\n"
                result += "- **'Show map'** - Display TPR distribution visualization\n"
                result += "- **'Summary statistics'** - Get detailed TPR statistics\n"
                result += "- **'Continue to risk analysis'** - Proceed to malaria risk ranking"
