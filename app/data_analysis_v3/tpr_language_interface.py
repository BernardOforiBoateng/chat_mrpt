...
class TPRLanguageInterface:
    ...
    def extract_command(
        ...
        # Phrase-level filtering: Detect data inquiry phrases BEFORE token extraction
        # This prevents false positives like "List all the columns" → extracting "all"
        data_inquiry_phrases = [
            'list all', 'show all', 'get all', 'view all', 'display all',
            'see all', 'print all', 'give me all', 'show me all',
            'list the', 'show the', 'get the', 'view the', 'display the',
            'what are the', 'what is the', 'tell me the', 'give me the',
            'show columns', 'list columns', 'get columns', 'view columns',
            'show data', 'view data', 'inspect data', 'explore data',
            'show rows', 'list rows', 'get rows', 'view rows'
        ]

        for phrase in data_inquiry_phrases:
            if phrase in message_lower:
                logger.info(f"🔍 Data inquiry phrase detected: '{phrase}' in '{message}' - routing to agent")
                return None
    ...
    def _llm_extract_command(
        ...
        if self._command_prompt is None:
            self._command_prompt = ChatPromptTemplate.from_messages([
                (
                    "system",
                    """You extract workflow commands from user messages.

The user is in a guided workflow and may be making a selection from valid options.

If the user's message contains a selection from the valid options, extract it.
If the user is asking a question, requesting information, or doing something else, return null.
