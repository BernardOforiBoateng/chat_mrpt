...
Return only the intent classification, nothing else."""
    ...
    def _handle_tpr_message(self, user_message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"🔍 TPR Router: Handler returned keys: {list(result.keys())}")
            logger.info(f"🔍 TPR Router: exit_data_analysis_mode = {result.get('exit_data_analysis_mode')}")
            logger.info(f"🔍 TPR Router: workflow = {result.get('workflow')}")
            ...
            # Ensure tools_used is tracked without clobbering existing metadata
            tools_used = result.get('tools_used') or []
            if isinstance(tools_used, list):
                if 'tpr_workflow' not in tools_used:
                    tools_used.append('tpr_workflow')
                result['tools_used'] = tools_used
            else:
                result['tools_used'] = ['tpr_workflow']

            # Preserve workflow/stage/visualisation metadata from handler response
            # while providing sensible defaults when missing.
            result.setdefault('status', 'success')
            result.setdefault('response', '')
            result.setdefault('visualizations', [])
            result.setdefault('download_links', [])

            logger.info(f"🔍 TPR Router: Final result keys: {list(result.keys())}")
            logger.info(f"🔍 TPR Router: Final exit_data_analysis_mode = {result.get('exit_data_analysis_mode')}")
