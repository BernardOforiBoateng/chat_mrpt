...
# Initialize database manager for routing logging
_db_manager = None

def get_db_manager():
    """Get or create the database manager for routing logging."""
    global _db_manager
    if _db_manager is None:
        try:
            _db_manager = DatabaseManager()
        except Exception as e:
            logger.warning(f"Could not initialize routing logging database: {e}")
            _db_manager = None
    return _db_manager
...
async def route_with_mistral(message: str, session_context: dict) -> str:
    ...
    if message_lower in ['thanks', 'thank you', 'bye', 'goodbye', 'ok', 'okay', 'sure', 'yes', 'no']:
        ...
        if db_manager:
            log_routing_decision(
                ...
                user_message=message,
                routing_decision="can_answer",
                routing_method="fast_track_smalltalk",
                keywords_matched=["smalltalk"],
                session_context=session_context
            )
        return "can_answer"
    ...
    if session_context.get('has_uploaded_files', False):
        ...
        if any(trigger in message_lower for trigger in analysis_triggers):
            matched_triggers = [t for t in analysis_triggers if t in message_lower]
            ...
            if db_manager:
                log_routing_decision(
                    ...
                    keywords_matched=matched_triggers,
                    session_context=session_context
                )
            return "needs_tools"
            ...
            if 'vulnerability' in message_lower and ('map' in message_lower or 'plot' in message_lower):
                ...
                if db_manager:
                    log_routing_decision(
                        ...
                        user_message=message,
                        routing_decision="needs_tools",
                        routing_method="keyword_match_distribution_viz",
                        keywords_matched=["distribution"],
                        session_context=session_context
                    )
                return "needs_tools"
            elif any(word in message_lower for word in ['box plot', 'boxplot', 'histogram', 'scatter', 'heatmap', 'bar chart']):
                matched_chart_types = [w for w in ['box plot', 'boxplot', 'histogram', 'scatter', 'heatmap', 'bar chart'] if w in message_lower]
                ...
                if db_manager:
                    log_routing_decision(
                        ...
                        user_message=message,
                        routing_decision="needs_tools",
                        routing_method="keyword_match_chart_type",
                        keywords_matched=matched_chart_types,
                        session_context=session_context
                    )
                return "needs_tools"
            # General visualization with data
            elif any(keyword in message_lower for keyword in ['plot', 'map', 'chart', 'visualize']):
                matched_viz_keywords = [k for k in ['plot', 'map', 'chart', 'visualize'] if k in message_lower]
                ...
                if db_manager:
                    log_routing_decision(
                        ...
                        user_message=message,
                        routing_decision="needs_tools",
                        routing_method="keyword_match_visualization",
                        keywords_matched=matched_viz_keywords,
                        session_context=session_context
                    )
                return "needs_tools"
        ...
        if any(trigger in message_lower for trigger in ranking_triggers) and 'ward' in message_lower:
            matched_ranking = [t for t in ranking_triggers if t in message_lower]
            ...
            if db_manager:
                log_routing_decision(
                    ...
                    user_message=message,
                    routing_decision="needs_tools",
                    routing_method="keyword_match_ranking",
                    keywords_matched=matched_ranking,
                    session_context=session_context
                )
            return "needs_tools"
        ...
        if any(trigger in message_lower for trigger in data_query_triggers):
            matched_data_queries = [t for t in data_query_triggers if t in message_lower]
            ...
            if db_manager:
                log_routing_decision(
                    ...
                    user_message=message,
                    routing_decision="needs_tools",
                    routing_method="keyword_match_dataquery",
                    keywords_matched=matched_data_queries,
                    session_context=session_context
                )
            return "needs_tools"
        ...
        if any(trigger in message_lower for trigger in intervention_triggers):
            matched_interventions = [t for t in intervention_triggers if t in message_lower]
            ...
            if db_manager:
                log_routing_decision(
                    ...
                    user_message=message,
                    routing_decision="needs_tools",
                    routing_method="keyword_match_intervention",
                    keywords_matched=matched_interventions,
                    session_context=session_context
                )
            return "needs_tools"
        ...
        if any(pattern for pattern in itn_param_patterns):
            ...
            if db_manager:
                log_routing_decision(
                    ...
                    user_message=message,
                    routing_decision="needs_tools",
                    routing_method="keyword_match_itn_params",
                    keywords_matched=["itn_parameter_response"],
                    session_context=session_context
                )
            return "needs_tools"
        ...
        if 'why' in message_lower and 'ward' in message_lower and ('rank' in message_lower or 'high' in message_lower or 'risk' in message_lower):
            ...
            if db_manager:
                log_routing_decision(
                    ...
                    user_message=message,
                    routing_decision="needs_tools",
                    routing_method="keyword_match_ward_analysis",
                    keywords_matched=["why", "ward"],
                    session_context=session_context
                )
            return "needs_tools"
...
Reply ONLY: NEEDS_TOOLS, CAN_ANSWER, or NEEDS_CLARIFICATION"""
        ...
        if decision == "NEEDS_TOOLS":
            ...
            if db_manager:
                log_routing_decision(
                    ...
                    user_message=message,
                    routing_decision="needs_tools",
                    routing_method="ollama_decision",
                    ollama_reasoning=response,
                    session_context=session_context
                )
            return "needs_tools"
            ...
            if any(ref in message_lower for ref in data_references):
                matched_refs = [r for r in data_references if r in message_lower]
                # Log routing decision
                if db_manager:
                    log_routing_decision(
                        db_manager=db_manager,
                        ...
                        user_message=message,
                        routing_decision="needs_tools",
                        routing_method="fallback_data_reference",
                        keywords_matched=matched_refs,
                        ollama_reasoning=f"Unexpected Ollama response: {decision}",
                        session_context=session_context
                    )
                return "needs_tools"
            ...
            if db_manager:
                log_routing_decision(
                    ...
                    user_message=message,
                    routing_decision="can_answer",
                    routing_method="fallback_general",
                    ollama_reasoning=f"Unexpected Ollama response: {decision}",
                    session_context=session_context
                )
            return "can_answer"
        ...
        if db_manager:
            log_routing_decision(
                ...
                ollama_reasoning=f"Routing error: {str(e)}",
                session_context=session_context
            )
        return "needs_clarification"
