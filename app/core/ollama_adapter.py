...
class OllamaAdapter:
    ...
    def __init__(self, base_url: str = None):
        ...
        if base_url:
            ...
            # Import arena config for GPU settings
            try:
                from app.config.arena_config import get_ollama_url, USE_GPU_INSTANCE
                self.base_url = get_ollama_url()
                if USE_GPU_INSTANCE:
                    logger.info("🚀 Arena using GPU-accelerated instance for faster inference")
            except ImportError:
                # Fallback if config not available
                aws_ollama_host = os.environ.get('AWS_OLLAMA_HOST')
                if aws_ollama_host:
                    self.base_url = f"http://{aws_ollama_host}:11434"
                    logger.info(f"🚀 Using GPU instance at {aws_ollama_host}")
                else:
