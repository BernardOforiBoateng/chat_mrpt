...
def create_tpr_map(tpr_results: pd.DataFrame, session_folder: str, state_name: str) -> bool:
    ...
    try:
        ...
        if state_gdf is None or state_gdf.empty:
            unique_states = master_gdf['StateName'].unique()[:10]
            logger.error(f"❌ No shapefile data found for '{state_name}' (normalized: '{normalized_state}')")
            logger.error(f"Available states (first 10): {unique_states}")
            return False
...
Try using 'all_ages' with 'both' test methods and 'all' facilities for the broadest analysis."""
            ...
            # Simplified, visual-first result
            result = f"""📊 **TPR Analysis Complete for {state_name}**

**Summary:** {summary['mean_tpr']}% average TPR across {summary['total_wards']} wards
**Coverage:** {summary['total_tested']:,} tests conducted ({summary['total_positive']:,} positive)"""

            # Only show top 3 high-risk wards for conciseness
            if summary['high_risk_wards']:
                threshold = summary.get('risk_threshold', summary['median_tpr'])
                result += f"\n\n**Top Priority Wards:** (above {threshold}% TPR)"
                for i, ward in enumerate(summary['high_risk_wards'][:3], 1):
                    result += f"\n{i}. {ward['WardName']}: {ward['TPR']}% TPR"

                # Mention if there are more
                if len(summary['high_risk_wards']) > 3:
                    result += f"\n...and {len(summary['high_risk_wards'])-3} more wards above {threshold}%"
            else:
                # Just show top wards without mentioning any threshold
                top_wards = tpr_results.nlargest(3, 'TPR')
                result += f"\n\n**Top Wards by TPR:**"
