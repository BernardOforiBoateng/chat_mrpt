...
# Worker processes
# Allow override via GUNICORN_WORKERS env; fallback to (cpu_count * 2) + 1

def _determine_worker_count() -> int:
    env_value = os.getenv('GUNICORN_WORKERS')
    if env_value:
        try:
...
timeout = 300  # Increased for complex analysis operations
graceful_timeout = 60
keepalive = 5
