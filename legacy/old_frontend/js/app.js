...
console.warn('🚨 APP.JS LOADING - ChatMRPT Main Application');

// Import all modules - UPDATED TO USE REFACTORED CHAT MANAGER
// Cache bust: v20250901debug
import chatManager from './modules/chat/chat-manager-refactored.js?v=20250901debug';
...
class ThemeManager {
    constructor() {
        // Theme is now handled by vertical nav, but keep this for compatibility
...
class ChatMRPTApp {
    constructor() {
        ...
        this.verticalNavManager = verticalNavManager;
    ...
    async initializeModules() {
        try {
            ...
            // Initialize TPR Download Manager
            this.tprDownloadManager = new TPRDownloadManager();
            
            // Initialize Privacy Notice Manager (singleton auto-initializes)
            this.privacyNoticeManager = privacyNoticeManager;

            // Use existing singleton modules (they auto-initialize)
            this.modules = {
                chat: this.chatManager,
                sidebar: this.sidebarManager,
                verticalNav: this.verticalNavManager,
                uploader: this.fileUploader,
                api: this.apiClient,
                privacy: this.privacyNoticeManager
            };
    ...
    async handleReportGeneration() {
        ...
        try {
            ...
            if (result.status === 'success') {
                // Success - show download link(s)
                if (result.download_links && result.download_links.length > 0) {
                    // New format with download_links array (ITN export)
                    let messageHtml = '<strong>Export Package Created Successfully!</strong><br><br>';
                    
                    result.download_links.forEach(link => {
                        messageHtml += `
                            <div class="mb-2">
                                <a href="${link.url}" class="btn btn-success btn-sm" download target="_blank">
                                    <i class="fas fa-download me-2"></i>${link.name}
                                </a>
                                <span class="text-muted ms-2">(${link.size})</span>
                            </div>
                        `;
                    });
                    ...
                    if (result.message && result.message.includes('**')) {
                        // Parse markdown-style message
                        const parsedMessage = result.message
                            .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
                            .replace(/\n/g, '<br>');
                        messageHtml = parsedMessage;
                    }
                    
                    this.chatManager.addSystemMessage(messageHtml);
                    
                } else if (result.download_url) {
                    // Old format with single download_url
                    this.chatManager.addSystemMessage(`
                    ...
                    this.chatManager.addSystemMessage(result.message || 'Report generated successfully!');
                }
    ...
    initializeLanguageFunctionality() {
        // Language is now handled by vertical nav
        // Keep this method for compatibility but listen to the custom event
    ...
    loadInitialState() {
        ...
        // Language is now set by vertical nav
...
window.privacyNoticeManager = app.privacyNoticeManager;
