...
import { MessageHandler } from './core/message-handler.js?v=20250829arena';
...
import welcomeAnimations from '../ui/welcome-animations.js';
...
export class ChatManager {
    constructor() {
        ...
        if (document.readyState === 'loading') {
            ...
            // DOM is already loaded, but still wait a tick to ensure all elements are rendered
    ...
    init() {
        ...
        console.warn('🚨 CHATMRPT IS LOADING - Version 2.0');
        ...
        if (!chatContainer || !messageInput || !sendButton) {
            ...
            // Retry once after a short delay if elements aren't found
            setTimeout(() => {
                const retryContainer = DOMHelpers.getElementById('chat-messages');
                const retryInput = DOMHelpers.getElementById('message-input');
                const retryButton = DOMHelpers.getElementById('send-message');
                
                if (retryContainer && retryInput && retryButton) {
                    this.initializeWithElements(retryContainer, retryInput, retryButton);
                } else {
                    console.error('❌ FATAL: Required DOM elements still not found after retry!');
                }
            }, 100);
            return;
        }
        
        this.initializeWithElements(chatContainer, messageInput, sendButton);
    }
    
    /**
     * Initialize with confirmed DOM elements
     */
    initializeWithElements(chatContainer, messageInput, sendButton) {
    ...
    setupResponseHandling() {
        ...
        document.addEventListener('messageResponse', async (event) => {
            ...
            // Handle Arena mode responses (dual model comparison)
            if (response.arena_mode === true && response.response_a && response.response_b) {
                console.log('🎮 Arena mode response detected');
                const dualElement = this.messageHandler.createDualMessageElement(
                    response.response_a,
                    response.response_b,
                    response.battle_id,
                    response.view_index
                );
                this.messageHandler.appendMessage(dualElement);
                
                // Store model names for later reveal (after voting)
                if (response.model_a && response.model_b) {
                    sessionStorage.setItem(`battle_${response.battle_id}_models`, JSON.stringify({
                        model_a: response.model_a,
                        model_b: response.model_b
                    }));
                }
                return;
            }
    ...
    loadWelcomeMessage() {
        ...
        if (this.messageHandler && this.messageHandler.chatContainer) {
            this.messageHandler.chatContainer.innerHTML = '';
            // Add class to help with styling
            this.messageHandler.chatContainer.classList.add('has-welcome-message');
        }
        
        // Get animated welcome message
        const welcomeHTML = welcomeAnimations.generateWelcomeMessage();
        
        // Add the message with a special class
        const messageElement = this.messageHandler.addAssistantMessage(welcomeHTML);
        if (messageElement) {
            messageElement.classList.add('welcome-message');
        }
        
        // Initialize interactions after message is added
        welcomeAnimations.initializeInteractions();
    }
    
    /**
     * Prompt user to upload data (called from welcome message button)
     */
    promptDataUpload() {
        // Close welcome and show upload modal
    ...
    /**
     * Load sample data (called from welcome message button)
     */
    loadSampleData() {
        const useSampleBtn = document.getElementById('use-sample-data-btn-modal');
        if (useSampleBtn) {
            // First open upload modal
            const uploadButton = document.getElementById('upload-button');
            if (uploadButton) {
                uploadButton.click();
                // Then click sample data after modal opens
                setTimeout(() => {
                    useSampleBtn.click();
                }, 300);
            }
