    ...
    >
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-2">
          <h3 className={`text-lg font-semibold ${isActive ? `text-${color}-700` : 'text-gray-900'}`}>
            {methodName} Analysis
          </h3>
          {score !== undefined && (
            <span className={`px-3 py-1 text-sm font-medium rounded-full ${
              isActive ? `bg-${color}-100 text-${color}-700` : 'bg-gray-100 text-gray-700'
            }`}>
        ...
        {ranking !== undefined && totalRanked !== undefined && (
          <div className="flex items-center mb-3">
            ...
            <span className="text-sm text-gray-600">
              Rank: <span className="font-semibold">{ranking}</span> of {totalRanked}
            </span>
          </div>
        )}
      ...
      {isExpanded && indicators.length > 0 && (
        <div className={`px-4 pb-4 pt-0`}>
          <div className={`rounded-lg p-3 ${isActive ? `bg-${color}-100` : 'bg-gray-50'}`}>
