...
echo ""
echo "1. Creating fixed arena_manager_redis.py locally..."
echo "==========================================="

# Create a fixed version with get_battle and storage_status
cat > app/core/arena_manager_redis_fixed.py << 'EOFFILE'
"""
...
echo "✅ Created fixed arena_manager_redis.py"

echo ""
echo "2. Deploying to production instances..."
echo "==========================================="
...
for IP in $INSTANCES; do
    ...
    scp -o StrictHostKeyChecking=no -i "$KEY_PATH" \
        ...
        ec2-user@$IP:/home/ec2-user/ChatMRPT/app/core/arena_manager_redis.py
    
    # Fix arena_routes.py on the server
    ssh -o StrictHostKeyChecking=no -i "$KEY_PATH" ec2-user@$IP << 'EOFREMOTE'
        cd /home/ec2-user/ChatMRPT
        
        # Backup arena_routes.py
        cp app/web/routes/arena_routes.py app/web/routes/arena_routes.py.backup
        
        # Fix the get_battle call in arena_routes.py
        sed -i 's/arena_manager\.battle_sessions\.get(battle_id)/arena_manager.get_battle(battle_id)/' app/web/routes/arena_routes.py
        
        # Add storage_status to the status endpoint
...
echo ""
echo "Fixed:"
echo "✅ Added get_battle() method to ArenaManager"
echo "✅ Added storage_status property to ArenaManager"
echo "✅ Updated arena_routes.py to use get_battle()"
echo "✅ Added storage_status to /api/arena/status endpoint"
echo ""
echo "Now test with: python test_arena_redis.py"