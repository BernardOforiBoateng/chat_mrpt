#!/bin/bash
# Deploy visualization reverts to match main branch

echo "========================================"
echo "Deploying Visualization Reverts"
echo "========================================"
echo ""
...
if [ $RESULT_1 -eq 0 ] && [ $RESULT_2 -eq 0 ]; then
    ...
    echo ""
    echo "Reverted to main branch configuration:"
    echo "  • Removed CreateVulnerabilityMapComparison from __init__.py exports"
    echo "  • Reverted _create_vulnerability_map to use visualization_service"
    echo ""
    echo "Current tool setup (matching main branch):"
    echo "  • CreateVulnerabilityMap - For composite method maps"
    echo "  • CreatePCAMap - For PCA method maps"
    echo "  • CreateCompositeScoreMaps - For composite with model breakdowns"
    echo "  • CreateVulnerabilityMapComparison - EXISTS but NOT exported (as intended)"
    echo ""
