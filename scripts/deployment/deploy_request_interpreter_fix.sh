#!/bin/bash
# Deploy request interpreter fix to AWS Production

echo "========================================"
echo "Deploying Request Interpreter Fix"
echo "========================================"
echo ""
...
echo "Target file: app/core/request_interpreter.py"
echo ""
...
if [ $RESULT_1 -eq 0 ] && [ $RESULT_2 -eq 0 ]; then
    ...
    echo ""
    echo "Fixed:"
    echo "  • Request interpreter now uses correct Pydantic tools"
    echo "  • Composite-only requests use CreateCompositeScoreMaps tool"
    echo "  • PCA requests use CreateVulnerabilityMap tool"
    echo "  • Comparison requests use CreateVulnerabilityMapComparison tool"
    echo ""
    echo "Users can now request:"
    echo "  • 'plot vulnerability map for composite score method' → Uses CreateCompositeScoreMaps"
    echo "  • 'show PCA vulnerability map' → Uses CreateVulnerabilityMap"
    echo "  • 'show vulnerability map' → Uses comparison tool"
    echo ""
