...
// FIXED VERSION - Simplified to ensure exports are created
// ----------------------------------------
...
// July 2018
var ndmi_2018_07 = loadAndMosaicTiles('NDMI', 2018, 7);
var ndwi_2018_07 = loadAndMosaicTiles('NDWI', 2018, 7);

Export.table.toDrive({
  collection: ndmi_2018_07.rename('NDMI').reduceRegions({
    collection: bufferedPoints,
    reducer: ee.Reducer.mean(),
    scale: 30,
    tileScale: 4
  }).map(function(f) { return f.set({'year': 2018, 'month': 7}); }),
...
Export.table.toDrive({
  collection: ndwi_2018_07.rename('NDWI').reduceRegions({
    collection: bufferedPoints,
    reducer: ee.Reducer.mean(),
    scale: 30,
    tileScale: 4
...
// August 2018
var ndmi_2018_08 = loadAndMosaicTiles('NDMI', 2018, 8);
var ndwi_2018_08 = loadAndMosaicTiles('NDWI', 2018, 8);

Export.table.toDrive({
  collection: ndmi_2018_08.rename('NDMI').reduceRegions({
    collection: bufferedPoints,
    reducer: ee.Reducer.mean(),
    scale: 30,
    tileScale: 4
  }).map(function(f) { return f.set({'year': 2018, 'month': 8}); }),
...
Export.table.toDrive({
  collection: ndwi_2018_08.rename('NDWI').reduceRegions({
    collection: bufferedPoints,
    reducer: ee.Reducer.mean(),
    scale: 30,
    tileScale: 4
...
// September 2018
var ndmi_2018_09 = loadAndMosaicTiles('NDMI', 2018, 9);
var ndwi_2018_09 = loadAndMosaicTiles('NDWI', 2018, 9);

Export.table.toDrive({
  collection: ndmi_2018_09.rename('NDMI').reduceRegions({
    collection: bufferedPoints,
    reducer: ee.Reducer.mean(),
    scale: 30,
    tileScale: 4
  }).map(function(f) { return f.set({'year': 2018, 'month': 9}); }),
...
Export.table.toDrive({
  collection: ndwi_2018_09.rename('NDWI').reduceRegions({
    collection: bufferedPoints,
    reducer: ee.Reducer.mean(),
    scale: 30,
    tileScale: 4
...
// October 2018
var ndmi_2018_10 = loadAndMosaicTiles('NDMI', 2018, 10);
var ndwi_2018_10 = loadAndMosaicTiles('NDWI', 2018, 10);

Export.table.toDrive({
  collection: ndmi_2018_10.rename('NDMI').reduceRegions({
    collection: bufferedPoints,
    reducer: ee.Reducer.mean(),
    scale: 30,
    tileScale: 4
  }).map(function(f) { return f.set({'year': 2018, 'month': 10}); }),
...
Export.table.toDrive({
  collection: ndwi_2018_10.rename('NDWI').reduceRegions({
    collection: bufferedPoints,
    reducer: ee.Reducer.mean(),
    scale: 30,
    tileScale: 4
...
// November 2018
var ndmi_2018_11 = loadAndMosaicTiles('NDMI', 2018, 11);
var ndwi_2018_11 = loadAndMosaicTiles('NDWI', 2018, 11);

Export.table.toDrive({
  collection: ndmi_2018_11.rename('NDMI').reduceRegions({
    collection: bufferedPoints,
    reducer: ee.Reducer.mean(),
    scale: 30,
    tileScale: 4
  }).map(function(f) { return f.set({'year': 2018, 'month': 11}); }),
...
Export.table.toDrive({
  collection: ndwi_2018_11.rename('NDWI').reduceRegions({
    collection: bufferedPoints,
    reducer: ee.Reducer.mean(),
    scale: 30,
    tileScale: 4
...
// December 2018
var ndmi_2018_12 = loadAndMosaicTiles('NDMI', 2018, 12);
var ndwi_2018_12 = loadAndMosaicTiles('NDWI', 2018, 12);

Export.table.toDrive({
  collection: ndmi_2018_12.rename('NDMI').reduceRegions({
    collection: bufferedPoints,
    reducer: ee.Reducer.mean(),
    scale: 30,
    tileScale: 4
  }).map(function(f) { return f.set({'year': 2018, 'month': 12}); }),
...
Export.table.toDrive({
  collection: ndwi_2018_12.rename('NDWI').reduceRegions({
    collection: bufferedPoints,
    reducer: ee.Reducer.mean(),
    scale: 30,
    tileScale: 4
...
Map.addLayer(ndmi_2018_07, {min: -1, max: 1, palette: ['red', 'yellow', 'green', 'cyan', 'blue']}, 
             'NDMI 2018-07', false);
Map.addLayer(ndwi_2018_07, {min: -1, max: 1, palette: ['brown', 'white', 'blue', 'darkblue']}, 
             'NDWI 2018-07', false);
...
print('');
print('For 2021, use the separate script or copy this pattern.');