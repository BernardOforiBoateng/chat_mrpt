...
SESSION_ID = f"test_tool_{int(time.time())}"
...
Rivers,45.3,7000000,410"""
    ...
    if response.status_code == 200:
        ...
        print(f"   Has data analysis file: {result.get('has_data_analysis_file', False)}")
        return True
    else:
...
def test_tool_calling():
    ...
    # Use the main chat endpoint which routes to Data Analysis V3 when data is uploaded
    response = requests.post(
