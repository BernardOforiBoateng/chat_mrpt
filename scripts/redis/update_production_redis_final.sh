#!/bin/bash
# Update Production Redis Configuration with Correct Virtual Environment
...
ssh -i "$SSH_KEY" "$STAGING_HOST" << 'TEST_REDIS'
    ssh -i ~/.ssh/chatmrpt-key.pem ec2-user@172.31.44.52 << 'PROD_TEST'
        ...
        # Use the production virtual environment
...
ssh -i "$SSH_KEY" "$STAGING_HOST" << 'VERIFY'
    ssh -i ~/.ssh/chatmrpt-key.pem ec2-user@172.31.44.52 << 'PROD_VERIFY'
        ...
        # Test Redis from within the application context using production venv
        /home/ec2-user/chatmrpt_env/bin/python << VERIFY_PY
import os
import sys
import redis
...
echo ""
echo "NEXT STEPS - PLEASE TEST:"
echo ""
echo "1. Clear browser cache and go to: http://chatmrpt-alb-319454030.us-east-2.elb.amazonaws.com/"
echo ""
echo "2. Test TPR workflow:"
echo "   a) Upload a TPR file"
echo "   b) Complete the analysis"
echo "   c) Click 'Download Processed Data' tab"
echo "   d) ✓ Verify download links appear and work"
echo ""
echo "3. Test ITN export:"
echo "   a) Run ITN distribution planning"
echo "   b) Export the package"
echo "   c) ✓ Verify dashboard HTML is included in the ZIP"
echo ""
